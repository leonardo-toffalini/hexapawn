...
class Game:
    ...
    def unmake_move(self, from_row, from_col, to_row, to_col, was_take, color):
        ...
        if was_take:
            ...
            tile.piece = Pawn((from_row, from_col), other_color, self.board, self.board.tile_size)
    ...
    def minimax(self, player:Color = Color.RED, depth:int = 9):
        ...
        if depth == 0 or self.check_winner():
            score = [-1, -1, -1, -1, self.evaluate()] # from_row, from_col, to_row, to_col, score
            return score
        ...
        for move in self.get_moves():
            ...
            score = self.minimax(Color.RED if player == Color.BLACK else Color.BLACK, depth - 1)
            self.unmake_move(to_row, to_col, from_row, from_col, player, take) # undo the move
