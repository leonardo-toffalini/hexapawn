from piece import Color, DEBUG, Piece
    ...
    def __init__(
        ...
        if board is None:
            ...
            for _ in range(self.num_tiles - 2):
                self.board.append([Color.EMPTY for _ in range(self.num_tiles)])
    ...
    def get_tile_from_pos(self, x: int, y: int) -> Tile | None:
        ...
        for tile in self.tiles_list:
            if (tile.x_index, tile.y_index) == (x, y):
                return tile
        raise Exception("no tile found in get_tile_from_pos in board.py :57")
