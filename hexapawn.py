"""
This file is outdate and not used anywhere.
"""
...
class Board:
    ...
    def displayBoard(self) -> None:
        print(
            f"{self._convert(self.board[0][0])}|{self._convert(self.board[0][1])}|{self._convert(self.board[0][2])}"
        )
        print("------")
        print(
            f"{self._convert(self.board[1][0])}|{self._convert(self.board[1][1])}|{self._convert(self.board[1][2])}"
        )
        print("------")
        print(
            f"{self._convert(self.board[2][0])}|{self._convert(self.board[2][1])}|{self._convert(self.board[2][2])}"
        )
    ...
    def getMoves(self, row: int, col: int) -> list:
        ...
        if self.board[row][col] == self.turn:
            if (
                row - self.turn in range(3)
                and col in range(3)
                and self.board[row - self.turn][col] == 0
            ):
                moves.append((row - self.turn, col))
            if (
                row - self.turn in range(3)
                and col - 1 in range(3)
                and self.board[row - self.turn][col - 1] == -self.turn
            ):
